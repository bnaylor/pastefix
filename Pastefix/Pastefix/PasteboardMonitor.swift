...
/// declared types alone before any content is touched, so content is never even read when it
/// won't be captured; `shouldCapture` runs again after the read, on the full candidate, for
/// filters (e.g. issue #10's app-exclusion) that need content `read` alone can populate, such as
/// `sourceBundleID`. Filters run in order at each stage; any `false` wins.
...
struct ConcealedTypeFilter: CaptureFilter {
    ...
    func shouldRead(types: [NSPasteboard.PasteboardType]) -> Bool { !Self.isConcealed(types) }
    func shouldCapture(_ candidate: CaptureCandidate, types: [NSPasteboard.PasteboardType]) -> Bool { !Self.isConcealed(types) }
}
...
final class PasteboardMonitor {
    ...
    private func tick() {
        ...
        // captured.
        guard filters.allSatisfy({ $0.shouldRead(types: types) }) else { return }
