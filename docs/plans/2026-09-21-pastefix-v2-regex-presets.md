...
- **Safety (as shipped — see spec amendments 1, 9, 10):** input cap `262_144` bytes → `TransformError.invalidInput("Text is too large for a regex preset (limit 256 KB)")`; **output** cap `8 × 262_144` bytes, checked per match inside the enumeration → `invalidInput("Replacement output is too large (limit 2 MB)")`; the replace runs off the main actor in a `withThrowingTaskGroup` raced against a **3 s** sleep → `TransformError.timeout`, but the race is only a second error path, not a bound (the group awaits its children). The bound is the `ContinuousClock` deadline the synchronous core checks **inside** `enumerateMatches(options: [.reportProgress])` — `.reportProgress` is what has the block called *during* a backtracking attempt, not only on a match, and is therefore the entire protection.
