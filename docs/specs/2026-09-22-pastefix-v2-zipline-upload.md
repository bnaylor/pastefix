...
| HTTP redirects on the upload | Follow a redirect that stays on the same origin (scheme, host and port unchanged); refuse every cross-origin one | `session.data(for:)` with no delegate follows redirects by default, so a 307/308 re-POSTs the clipboard — and the `authorization` header — to wherever the server points, and whether CFNetwork strips that header across origins is version-dependent. `SameOriginRedirectPolicy` decides instead, passed as a per-**task** delegate to `session.data(for:delegate:)`. Refuse rather than forward with the token stripped: the token is only one of the two secrets on this request, a 307/308 re-sends the **body** regardless, and there is no useful version of "send the text to a host the user never typed". `URLSessionTaskDelegate` does expose authentication-challenge callbacks — the inherited session-level one plus its own task-level one — but `SameOriginRedirectPolicy` implements neither, so this does not reopen the self-signed-certificate position above — a challenge still falls through to default handling and an untrusted certificate still fails. The cost is a reverse proxy that 301s `http`→`https`, which is cross-origin and refused; ATS already forces the user to type the `https` URL for any named host, so this costs close to nothing. A refused redirect surfaces as `.server(status:)` with a message naming what happened, not a bare 3xx. |
| App Transport Security | `NSAllowsLocalNetworking` in the app's `Info.plist`; **never** `NSAllowsArbitraryLoads` | The Upload tab tells the user that LAN and Tailscale addresses are the normal deployment, so what ATS actually permits over plain `http` is part of the feature working at all. Measured on macOS 26.3.1, a signed `.app` with and without the key: the stock default **already** permits `http` to an IP literal (192.168.x, 10.x, a Tailscale 100.x CGNAT address, 127.0.0.1) and to a `.local` name, and **both** settings refuse `http` to a dotted public-looking FQDN — a MagicDNS name such as `box.tailnet.ts.net` included — with `NSURLErrorAppTransportSecurityRequiresSecureConnection` (-1022). So the key buys nothing observable on this OS; it is set to state the intent and to keep the local case working if the implicit exemptions tighten. A blanket exception is refused outright: this app uploads the clipboard and an API token, and disabling transport security globally to spare one deployment is the wrong trade. What changes as a result is the settings caption, which now says which addresses work unencrypted instead of promising a MagicDNS name will — the remedy there is `https`, and Tailscale issues a real certificate for it. |
