...
rewrite, so this is expected to be cheap, but **the implementation plan opens
with a measurement** of the real cost curve above 256 KB. If a megabyte costs
seconds of CPU after dismissal, that becomes a cancellation check inside
`SecretDetector` and a progress bar instead of a spinner — a design change, and
better found before the UI is built than after.
