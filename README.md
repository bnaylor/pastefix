...
- **Large-buffer safety** — every transform has an input cap and a timeout (1 MB and 3 s by default). Markdown → Rich Text is limited to 64 KB. Clean URL Tracking and URL → Markdown Link are limited to 256 KB, because URL detection itself stops scanning at 256 KB: `Detected: URL` will not appear on a larger buffer, and those two transforms refuse with a banner naming the limit rather than running blind. Redact Secrets and the regex presets keep their own 256 KB caps. On a very large paste the panel appears first; the `Detected:` label and any secret badges fill in a moment later once the scan lands.
