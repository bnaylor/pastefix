...
Pastefix/                             # the Xcode app (KeyboardShortcuts + Sparkle dependencies only)
  ...
  Pastefix/                           # app sources (system glue only, no unit tests)
    ...
    PasteboardMonitor.swift           # polls changeCount 2x/sec; builds CaptureContext from FrontmostAppTracker before each read, refreshes it after; two-stage CaptureFilter chain (ConcealedTypeFilter + AppExclusionFilter) (Critical Invariant 12); TIFF->PNG conversion runs detached, re-checking change count + filters on the way back (#32)
    HistoryOverlayView.swift          # ⌘⇧V/⌘Y overlay: HistorySearch-ranked list, thumbnails (blob read + decode off-main, installed by item id even if the requesting row is gone), ↵/⌘↵/⌘⌫/⌘P/⇧↵/Esc; Pinned/History sections
