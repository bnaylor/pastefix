...
Sources/PastefixCore/
  ...
  Upload/
    UploadLimits.swift                # maxPayloadBytes (16 MB admission cap), idleTimeout (60 s), resourceTimeout (600 s) + minimumThroughputBytesPerSecond — the cap and the whole-transfer timeout are one decision, asserted by UploadLimitsTests
    ZiplineUpload.swift               # ZiplineUpload request value (throwing init: the single choke point for the file extension, ZiplineFileExtension.canonical — [a-z0-9._+-]{1,16}, refused not sanitised), ZiplineExpiry, SecretDisposition, UploadPayload.text (the one place that decides which bytes leave), ZiplineUploadError
    ZiplineV4Headers.swift            # ZiplineV4Headers.headers(for:) — the only place a v4 header spelling appears
    ZiplineClient.swift               # ZiplineUploading protocol + URLSessionZiplineClient: multipart body (takes the whole validated ZiplineUpload), endpoint normalisation, response/error mapping; idle timeout and resource timeout are separate values
...
Sources/PastefixAppCore/              # app pure model (depends on PastefixCore, NO third-party deps)
  ...
  Upload/
    ZiplineTokenStore.swift           # ZiplineTokenStore protocol; KeychainTokenStore (SecItem*, service net.scromp.Pastefix.zipline, untested); InMemoryTokenStore for tests
...
Pastefix/                             # the Xcode app (KeyboardShortcuts + Sparkle dependencies only)
  ...
  Pastefix/                           # app sources (system glue only, no unit tests)
    PastefixApp.swift                 # @main MenuBarExtra + NSApplicationDelegateAdaptor; owns HistoryStore + PasteboardMonitor + FrontmostAppTracker, menu-bar Clipboard History toggle + pause glyph, third hotkey (summonUpload), flush on terminate
    AppModel.swift                    # @MainActor ObservableObject: registry+document+clipboard+history; load(_:)/copyBack(_:), historyOverlayRequested, uploadOverlayRequested
    HotkeyName.swift                  # KeyboardShortcuts recorder + display helper; summonPastefix (⌘⇧C) + summonHistory (⌘⇧V) + uploadToZipline (⌘⇧U)
    ...
    SettingsView.swift                # SwiftUI Settings window (General/Privacy/Snippets/Presets/Upload/Shortcut/Transforms tabs); Privacy has the History section (moved from General) + Excluded Apps list + two-option Clear dialog, Snippets has Accessibility status + per-pin title/recorder/Unpin rows (shortcutValidation blocks combos already bound to another snippet or to the summon shortcuts, and the Shortcut tab validates against snippets in turn), Presets tab delegates to PresetsSettingsView, Upload tab delegates to UploadSettingsView, Shortcut a second recorder
    ...
    UploadSettingsView.swift          # Upload tab: server URL draft (commit-on-submit/blur/teardown), token SecureField + explicit Set button (covers ⌘Q mid-edit, which teardown-commit does not) writing straight to ZiplineTokenStore and never reading it back, Clear Token, expiry/burn/extension defaults bound live to SettingsStore
    ZiplineServerURL.swift            # ZiplineServerURL.parse(_:) — the one server-URL trim/scheme/host check, shared by the overlay and the Upload tab; deliberately does NOT reuse MarkdownLink.isFetchable (that guard is for untrusted clipboard content, not a URL the user typed into their own Settings)
    ...
    PanelView.swift                   # editor + action bar + full-panel ⌘K/history/upload overlay host + sidebar column + Esc owner; toolbar pin button/⌘⇧P opens a title popover (pinCurrentBuffer); Preview toggle (⌘⇧M) swaps the editor for MarkdownPreviewView, 150ms-debounced re-render on buffer/undo/redo change, Esc closes the preview before Cancel; the three overlays are mutually exclusive and the upload overlay disables the editor underneath it (it uploads a snapshot taken on open) and is keyed by a generation counter so a repeat ⌘⇧U rebuilds it instead of reusing stale server/token/buffer state
    ...
    UploadOverlayView.swift           # ⌘⇧U overlay: configure-state check (server + Keychain token) before anything is drawn, then expiry/burn/extension controls plus an inline secret gate over a snapshot of the buffer taken on open — scanned via SecretDetector.scanIgnoringSizeCap in a detached task (Upload disabled until it resolves), Redact preselected, UploadPayload.text decides what actually leaves; success writes the clipboard and shows Copy/Open; dismissing mid-upload cancels the client Task but cannot recall server acceptance (known behaviour, see spec)
...
13. **Clipboard content leaves the machine only through a surface that has scanned it in full, and the Zipline API token is Keychain-only.** Two properties, both established the hard way on the first feature that sends the clipboard anywhere (Plan 13, PR #59). *The scan:* "not scanned" is never "clean". Any path that sends clipboard content off the machine — upload, share, sync, a future anything — scans the whole buffer first (`SecretDetector.scanIgnoringSizeCap`, no 256 KB cap), off the main actor, inside an explicit admission cap that **refuses** what it cannot scan rather than sending bytes nobody looked at, and it *gates* the send rather than merely informing the user. An empty match list from an unscanned buffer is not evidence of anything. *The token:* it lives in the Keychain and nowhere else — never in `UserDefaults`, never in a log line, never in an error message or a diagnostic string. `TokenStoreError` carries an `OSStatus`, not the token; the client's `.server` message names the request URL, where the token is a header rather than part of the URL. A redirect off the configured origin is refused outright rather than followed with the `authorization` header stripped, because the body is the second secret. The full rule, and what each clause cost to learn, is the "clipboard leaves the machine" bullet under Patterns; this entry is the binding statement.
...
- **The clipboard leaves the machine only through a surface that has scanned it in full, and "not scanned" is never "clean"** (Plan 13): the Zipline upload path is the first thing in this app that sends clipboard content off the machine, and it earns that by calling `SecretDetector.scanIgnoringSizeCap(_:)` — the whole buffer, no 256 KB cap — before Upload is even enabled, with Redact preselected over Send as-is. There is one admission cap above that scan and it is a *refusal*, not a shortcut: a buffer over `UploadLimits.maxPayloadBytes` (16 MB) is never scanned and never sent, and the overlay says so with the actual size and the limit — the cap bounds the work (an uncancellable detached scan, plus a redacted copy and a multipart body, ~3× resident), while what it must never do is produce a clean verdict on bytes nobody looked at. The cap and the client's *resource* timeout are one decision: `UploadLimits` ties them together so the path cannot admit a size the client has no time to finish sending. Any future upload, share, or sync path gets the same gate before it gets a network call: an uncapped scan within an explicit admission cap, refusing above it, gating the send rather than merely informing it, off the main actor.
...
*Zipline upload (Plan 13) — the first path that sends clipboard content off the machine, so most of these are about what "scanned" actually has to mean:*
- **`SecretDetector.scan(_:)` silently returns `[]` above its 256 KB cap** (`c38201f`): a naive "scan before upload" would have called the existing `scan(_:)` and reported "No secrets found" on the largest, most consequential pastes without looking at a single byte of them — the exact false all-clear Plan 11 already named as the worst outcome a cap can produce. `scanIgnoringSizeCap(_:)` was split out as the uncapped entry point for the one deliberate off-main caller the upload path is; `scan(_:)` itself keeps the 256 KB guard, unchanged, for every main-actor caller. A separate, deliberately awkward name rather than a defaulted parameter, so the one call site that skips the cap reads as the exception it is.
- **`x-zipline-format` is the *filename* format, not syntax highlighting** (`e522b2d`, spec correction from reading `diced/zipline` v4.7.0 rather than the original issue): a header that reads like the one you want is not the one you want. v4 picks highlighting from the file **extension**; `x-zipline-format` only controls how the server names the stored file (`random`/`uuid`/`date`/`name`/`gfycat`). Verify a third party's header contract against its source, not its name.
- **`URL.appendingPathComponent` on a user-supplied server URL doubled the path** (`e1cc316`): anyone pasting the full `.../api/upload` endpoint — which is exactly the string Zipline's own iShare-era config puts in front of a migrating user — got `.../api/upload/api/upload` and a 404 with no message. `URLSessionZiplineClient.endpoint(for:)` normalises instead: strip trailing slashes, append `api/upload` only if the path doesn't already end in it. Normalise a user-entered base URL before composing it; an unexplained 404 is not a diagnosable failure.
- **A measurement that chunks its input to stay under a cap cannot detect superlinear cost above that cap** (`ab3d4fa` → `c38201f`): Task 1's first pass split the corpus into ≤256 KB pieces and summed `scan()` calls over them, because no uncapped entry point existed yet. That makes the total mechanically `chunks × constant` by construction — it is linear no matter what the underlying regex engine does on a multi-MB `NSString`, which is the one thing the measurement exists to catch. Replaced with single, whole-buffer calls to `scanIgnoringSizeCap` once that entry point existed (256 KB in 0.058s, 1 MB in 0.222s, 4 MB in 0.854s — genuinely linear). Measure the path you will actually ship, not a shape of it that happens to fit the tool you already had.
- **A regression test that only inspects state it wrote itself is a tautology** (`75861c7`, caught before merge): the first `tokenIsNotInDefaults` set exactly one property (`ziplineServerURL`) and then checked `UserDefaults` for exactly the keys that one assignment could have produced — a test that cannot fail no matter what else `SettingsStore` does, including if someone added a real token property. Fixed by reassigning every persisted property (not just the Zipline ones) and scanning every resulting key for credential-shaped words (`token`, `secret`, `password`, `credential`, `apikey`, `auth`), so the guard covers every key the type can currently write. Still cannot catch a future property whose own assignment is missing from that list — a `didSet` that never runs never writes a key for the scan to see — so a new persisted property needs its assignment added to the test, not just its declaration.
- **Turning a stored property into a computed one that answers `[]` while it is still working is a source-compatible semantic change, and a merge will not flag it** (merge of PR #53 into this branch): Plan 14 made `PasteDocument.detectedKinds` return `[]` whenever `detection` is `.pending`, and `UploadOverlayView.init` reads it synchronously to seed the file extension. ⌘⇧U re-snapshots the clipboard immediately before the overlay appears, so detection is *always* pending at that instant and every JSON upload silently defaulted to `.txt` — no conflict, no crash, no failing test, because the three Swift files involved auto-merged cleanly. A value that used to be available at `init` and is now delivered later needs its readers moved from "read it once" to "observe it" (`ZiplineUpload.extensionSeed(setting:detectedKinds:userHasEditedField:)`, asked at `init` *and* on the detection result), and anything the user can have edited in the meantime must be exempt from the late write. Read every auto-merged file when either side changed what a shared property *means*; a clean merge only proves the text reconciled.
- **A doc comment addressed to your feature by name is still advice about a specific mechanism, not a licence** (same merge): `AppModel.isDetecting` says the Zipline gate "must wait for this before treating an empty `secretMatches` as 'no secrets'" — true for a gate that reads `secretMatches`, and this overlay deliberately does not. It runs its own `SecretDetector.scanIgnoringSizeCap` because `secretMatches` comes from the 256 KB-capped `scan`, so following the comment would have swapped an uncapped scan for a capped one and reintroduced the false all-clear the whole feature exists to prevent. The gate now carries a comment saying so, because the next reader will see the same invitation to simplify.
...
| 13 — Zipline upload | ZiplineClient, upload overlay, Keychain token, Upload tab | 🟡 in progress, branch `feat/zipline-upload` — [spec](docs/specs/2026-09-22-pastefix-v2-zipline-upload.md), [plan](docs/plans/2026-09-22-pastefix-v2-zipline-upload.md) |
