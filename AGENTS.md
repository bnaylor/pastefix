...
Pastefix/                             # the Xcode app (KeyboardShortcuts + Sparkle dependencies only)
  ...
  Pastefix/                           # app sources (system glue only, no unit tests)
    ...
    HistoryOverlayView.swift          # ⌘⇧V/⌘Y overlay: HistorySearch-ranked list, thumbnails (blob read + decode off-main, keyed by item id), ↵/⌘↵/⌘⌫/⌘P/⇧↵/Esc; Pinned/History sections
