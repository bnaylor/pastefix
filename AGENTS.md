...
Pastefix/                             # the Xcode app (KeyboardShortcuts + Sparkle dependencies only)
  ...
  Pastefix/                           # app sources (system glue only, no unit tests)
    ...
    PanelController.swift             # floating resizable NSPanel host (+ sidebar-driven resize)
    PanelView.swift                   # editor + action bar + ⌘K overlay host + sidebar column + Esc arbitration
...
- **Browsing UIs stay dumb:** the ⌘K palette and the sidebar both read `enabledTransformers()` and render whatever a pure AppCore function hands back — ranking (`TransformSearch`), grouping (`SidebarGrouping`), and applicable-first ordering (`PaletteOrdering`) are pure functions in `PastefixAppCore`, not view logic. A view should never re-sort or re-filter the list itself.
...
| 4 — Action bar | ⌘K palette, sidebar, categories | 🟡 in review on `feat/action-bar`, PR pending |
